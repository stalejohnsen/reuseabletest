param (
  [Parameter()]
  [String]$NonRootParentManagementGroupId = "$($env:TOP_LEVEL_MG_PREFIX)",
...
if ($NonRootParentManagementGroupId -ne '') {
  $inputObject = @{
    ...
    TemplateParameterFile                 = $TemplateParameterFile
    parTopLevelManagementGroupPrefix      = $NonRootParentManagementGroupId
    parTopLevelManagementGroupDisplayName = "$($env:TOP_LEVEL_MG_DISPLAYNAME)"
    WhatIf                                = $WhatIfEnabled
    Verbose                               = $true
  }
